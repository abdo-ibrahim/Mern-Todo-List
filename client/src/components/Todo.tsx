...
const Todo: React.FC<TodoProps> = React.memo(({ todo, fetchTodos }) => {
  ...
  const handleToggle = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await api.put(`${todoApi}/toggleDone/${id}`);
  ...
  const handleEdit = async (id: string, e: React.MouseEvent) => {
    ...
    if (isEdit) {
      try {
        await api.put(`${todoApi}/updateTodo/${id}`, {
          title,
        });
  ...
  return (
    <div className="todo p-2 bg-white w-full  rounded-lg mt-2 flex justify-between items-center cursor-pointer" onClick={(e) => handleToggle(todo._id, e)}>
      {!isEdit ? <p className={`${todo.done ? "line-through " : ""} font-bold flex-1`}>{todo.title}</p> : <Input type="text" className="mr-2" value={title} onChange={(e) => setTitle(e.target.value)} />}
